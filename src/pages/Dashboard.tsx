...
import { Calendar, Award, DollarSign, CheckCircle, Clock } from "lucide-react";
...
export default function Dashboard() {
  ...
  const loadData = async () => {
    ...
    const [todayRes, monthRes, totalRes, todayListRes] = await Promise.all([
      ...
      supabase.from("atendimentos").select("valor_repasse").eq("status", "concluido").gte("data_hora", monthStart).lt("data_hora", monthEnd),
    ...
    setMetrics({
      ...
      mesFaturamento: monthData.reduce((s, a) => s + (Number(a.valor_repasse) || 0), 0),
  ...
  const metricCards = [
    { icon: Calendar, label: "Atendimentos hoje", value: metrics.hoje, color: "text-secondary" },
    { icon: Award, label: "Certificados no mês", value: metrics.mesEmitidos, color: "text-accent" },
    { icon: DollarSign, label: "Faturamento do mês", value: formatCurrency(metrics.mesFaturamento), color: "text-accent" },
    { icon: CheckCircle, label: "Atendimentos concluídos", value: metrics.totalConcluidos, color: "text-secondary" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <Card className="lg:col-span-3 shadow-card rounded-xl">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg">Agenda de Hoje — {formatDateExtended(new Date())}</CardTitle>
          </CardHeader>
          <CardContent>
            {todayAtendimentos.length === 0 ? (
              <div className="text-center py-12">
                ...
                <Button variant="outline" className="mt-4" onClick={() => navigate("/agenda")}>Ir para a Agenda</Button>
              </div>
            ) : (
              <div className="space-y-3">
                {todayAtendimentos.map((a) => (
                  <div key={a.id} className="flex items-center gap-4 p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                    <span className="text-sm font-semibold text-secondary min-w-[50px]">{formatTime(a.data_hora)}</span>
                    <span className="font-medium text-foreground flex-1">{a.clientes?.nome || "—"}</span>
                ...
                <Button variant="outline" className="w-full mt-2" onClick={() => navigate("/agenda")}>Ver agenda completa</Button>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="lg:col-span-2 shadow-card rounded-xl">
          ...
          <CardContent className="space-y-3">
            {todayAtendimentos.length === 0 ? (
              ...
              todayAtendimentos.map((a) => (
                <Card key={a.id} className="shadow-card rounded-lg">
